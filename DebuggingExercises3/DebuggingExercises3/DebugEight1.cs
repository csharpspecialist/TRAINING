...
class DebugEight1
{
   public static void EightOne()
    {
        int orderNum, quantity;
        double total;
        const double PRICE_EACH = 3.99;
        GetData (out orderNum, out quantity);// add out 2 times
        total = quantity * PRICE_EACH; //ch from PRICEEACH TO PRICE_EACH
        WriteLine("Order # {0}. Quantity ordered = {1}",
            orderNum, quantity);// add )
      WriteLine("Total is {0}", total.ToString("C"));
        ReadLine();  //add readLine
   }
